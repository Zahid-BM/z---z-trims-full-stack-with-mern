...
const ProfileRow = ({ profile, index }) => {
    ...
    const handleMakeAdminBtn = () => {
        const url = `https://zandz-trims.onrender.com/profiles/admin/${email}`;
        fetch(url, {
            method: 'PUT',
            headers: {
    ...
    const handleRemoveBtn = id => {
        ...
        if (userConfirmation) {
            const url = `https://zandz-trims.onrender.com/profiles/${id}`;
            fetch(url, {
                method: 'DELETE'
            })
                .then(res => res.json())
    ...
    return (
        ...
        <tr key={profile?._id}>
            ...
            <td className='bg-secondary'><img className='cursor-selector' onClick={() => handleRemoveBtn(profile?._id)} src={cancel} alt="" /></td>
        </tr>
